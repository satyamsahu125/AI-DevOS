...
proj = api("POST", "/projects", {
    "name": "Image Captioner CLI",
    "description": (
        "Create a command-line tool in Python that monitors a specified directory for new image files (.png, .jpg, .jpeg) and, upon detection, uses a local AI model to generate a descriptive caption for each image, appending the filename and caption to a captions.csv file."
    )
})
log(f"Project response: {json.dumps(proj, indent=2)}")

project_id = proj.get("project_id") or proj.get("id") or proj.get("project", {}).get("project_id")
if not project_id:
